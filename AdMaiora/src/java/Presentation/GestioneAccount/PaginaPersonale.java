...
public class PaginaPersonale extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet PaginaPersonale</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet PaginaPersonale at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if(null != sessione) {
            UtenteGenerico utente = managerUtente.getById((String) sessione.getAttribute("nomeUtente"));
            if(null != utente) {
                ...
                if("MODERATORE".equals(utente.getRuolo())) {
                    ManagerRichiesta managerRichiesta = new ManagerRichiesta();
                    List<RichiestaCreazione> richieste = managerRichiesta.getAllPending();
                    request.setAttribute("listaRichieste", richieste);
                }
            }
        }
